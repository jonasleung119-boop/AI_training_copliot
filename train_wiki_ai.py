...
for topic in topics:
    try:
        ...
        print(f"Disambiguation error for '{topic}': {e.options[:5]}")
    except wikipedia.exceptions.PageError:
        print(f"Page not found for '{topic}'. Skipping.")
...
# Split text into non-overlapping chunks; each chunk is 4x block_size characters
# (characters per token ≈ 4), so each chunk fills roughly one block of tokens
chars_per_chunk = block_size * 4
chunks = [full_text[i:i + chars_per_chunk] for i in range(0, len(full_text), chars_per_chunk)]
